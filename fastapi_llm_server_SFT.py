...
TRAINED_PATH = "./gemma3_unsloth_ckc_fine_sft"
DATA_PATH = "mitre_ckc_finetune.jsonl"
MAX_SEQ_LENGTH = 8192   # 8192 -> 4096 -> 2048 -> 1024
...
def infer(request: InferenceRequest):
    ...
    try:
        ...
        return {"error": f"Error during inference: {str(e)}", "model_used": ACTIVE_MODEL_TYPE}
...
def train_model(config: TrainConfig):
    def run_training():
        ...
        # --- データセットの読み込みとフォーマット ---
        training_status["status"] = "Loading and formatting dataset..."
        logger.info(f"Loading dataset from {DATA_PATH}")


        # [MODIFIED] Hugging Faceのdatasetsライブラリを使用してJSONLファイルを直接ロードします。
        # これにより、手動でのファイル読み込みや複雑なフォーマット関数が不要になります。
        try:
            full_dataset = load_dataset("json", data_files=DATA_PATH, split="train")
            logger.info(f"Successfully loaded dataset from {DATA_PATH}. Number of examples: {len(full_dataset)}")
        except Exception as e:
            log_msg = f"Failed to load dataset from {DATA_PATH}. Error: {e}"
            training_status.update({"status": "Error", "logmsg": [log_msg]})
            logger.error(log_msg)
            return
        ...
        formatted_dataset = full_dataset.map(formatting_prompts_func, batched=True,)
        ...
        # --- 診断: データセットの総トークン数を計算 ---
        # packing=True を使用する際、この値がmax_seq_lengthより小さいと学習ステップが0になるため
        try:
            total_tokens = sum(len(TOKENIZER.encode(sample["text"])) for sample in train_dataset)
            logger.info(f"DIAGNOSTIC: Total number of tokens in the training dataset is approximately: {total_tokens}")
            if total_tokens < MAX_SEQ_LENGTH:
                logger.warning(
                    f"\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
                    f"DIAGNOSTIC WARNING: The total number of tokens in your training set ({total_tokens}) "
                    f"is less than max_seq_length ({MAX_SEQ_LENGTH}).\n"
                    f"With 'packing=True', this will result in 0 training samples and 0 training steps.\n"
                    f"To fix this, please either use a larger dataset or set 'max_seq_length' to a value "
                    f"smaller than the total token count (e.g., 512).\n"
                    f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                )
        except Exception as e:
            logger.error(f"DIAGNOSTIC: Failed to calculate total tokens. Error: {e}")
        ...
        training_args = SFTConfig(
            ...
            # max_steps = 500, # num_train_epochsが優先されるため、UIからのエポック数指定を有効にするためにコメントアウト
            warmup_ratio = 0.1,
        ...
        trainer = CustomSFTTrainer( # Use the custom SFTTrainer
            ...
            # packing=Falseの場合、max_seq_lengthを超える長いサンプルは破棄されるため、サンプル数が0になる可能性がある
            packing=True, # packing=Trueに設定し、長いサンプルをチャンクに分割して効率的に学習させる
